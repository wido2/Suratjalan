...
class NomorSuratJalan extends Controller
{
    public static function generate($input,  $pad_len = null, $pad_string = null, $prefix = null, $year = null)
    {
        $prefix = empty($prefix) ? config('nomorSuratJalan.prefix') : $prefix;
        $prefix = Str::upper($prefix);
        $pad_len =  empty($pad_len) ? config('nomorSuratJalan.pad_len') : $pad_len;
        $pad_len = intval(2);
        // check is year true from config
        if(config('nomorSuratJalan.year') == true){
            $year = empty($year) ? config('nomorSuratJalan.year_val') : $year;
            $year = intval($year);
        }else{
            $year = '';
        }
        ...
        $input = intval($input);
        $pad_direction =config('nomorSuratJalan.pad_direction');
        $pad_string = empty($pad_string) ? config('nomorSuratJalan.pad_string') : $pad_string;
